...
exports.getAllOrders = async (req, res) => {  // exports makes function available to other files (like routes). // async & await help handle database operations that take time
    ...
    const orders = await prisma.order.findMany();  // prisma get all orders from database
...
exports.getOrderById = async (req, res) => {
    ...
    const order = await prisma.order.findUnique({where: {id}});  // getting one specific order that matches our id variable
...
// POST: creating an order & adding it to the orderss list
exports.createOrder = async (req, res) => {
    const { customer, total, status } = req.body;  // extract these specific fields (properties) from (obj) req.body (the json data sent in the post request)
    const newOrder = await prisma.order.create({  // create a new order in the database
        data: { customer, total, status }  // actual data to save
    })
